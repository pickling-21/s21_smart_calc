...
enum valid_erorrs validator(const char* str, char** result,
  ...
  *result = malloc(sizeof(char) * len_str);
  if (result != NULL) {
    ...
    error = brackets_okey(*result) == true ? VA_OK : VA_BRACKETS;
    if (error == VA_OK) replace_unary_minus(*result);
  } else {
    error = VA_MEMORY;
  }
  return error;
...
bool brackets_okey(const char* str) {
  ...
  for (; *str != '\0' && res == true; str++) {
    // printf("%d %s\n", i, str);
    if (is_open_bracket(*str)) stack_operators_push(s, CAT_LEFT);
    if (is_close_bracket(*str)) {
      if (stack_operators_is_empty(s)) {
        res = false;
      } else if (stack_operators_last(s) == CAT_LEFT) {
...
// bool simple_opers_okey(const char* str) {
//   bool res = true;
//   for (char* end = NULL; *str != '\0' && res == true;) {
//     if (operator_check(str, &end) == CAT_NONE) {
//       res = false;
//     } else {
//     }
//   }
...
void print_error(enum valid_erorrs err) {
  char* errr_message[] = {[VA_BRACKETS] = "неправильное количество скобок",
                          [VA_MEMORY] = "проблема с памятью",
                          [VA_OK] = "Вcе ок"};
  printf("%s\n", errr_message[err]);
}